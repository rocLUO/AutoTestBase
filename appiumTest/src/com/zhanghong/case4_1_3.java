...
public class case4_1_3 extends TestInfo {
	...
	public void case4_1_3() throws InterruptedException {
		...
		try {
			...
			// (new TouchAction(driver)).tap(202, 894).perform();
			// (new TouchAction(driver)).tap(509, 898).perform();
			// (new TouchAction(driver)).tap(51, 668).perform();
			// (new TouchAction(driver)).tap(671, 672).perform();
			// (new TouchAction(driver)).tap(664, 672).perform();

			// 点击第二个
			Thread.sleep(3000);
			ClickXY(537, 1090, "点击工作");
			// ClickId("com.android.ayplatform.release:id/item_workbench_search_result_app_with_submenu_rootMenu");
			// driver.findElementByXPath("//android.widget.GridView/android.widget.LinearLayout[1]").click();

			Thread.sleep(1000);
			Back();
			// 点击新建工作
			// ClickId("com.android.ayplatform.release:id/activity_flow_new");
			// driver.findElementByXPath(
			// "//android.view.View/android.support.v7.widget.RecyclerView/android.widget.LinearLayout[2]").click();
			Thread.sleep(500);
			Back();
